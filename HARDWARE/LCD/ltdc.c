...
#else
	u16 ltdc_lcd_framebuf[1280][800] __attribute__ ((section (".ext_sram")));	//定义最大屏分辨率时,LCD所需的帧缓存数组大小
#endif
...
u16 LTDC_PanelID_Read(void)
{
	...
	bsp_gpio_init(GPIOG,SYS_GPIO_PIN6,SYS_GPIO_MODE_IN,0,0,SYS_GPIO_PUPD_PU); 			//PG6上拉输入 
	bsp_gpio_init(GPIOI,SYS_GPIO_PIN2|SYS_GPIO_PIN7,SYS_GPIO_MODE_IN,0,0,SYS_GPIO_PUPD_PU); 		//PI2/PI7上拉输入
	idx=PGin(6);					//读取M1
	idx|=PIin(2)<<1;				//读取M0
	idx|=PIin(7)<<2;				//读取M2 
...
{   
	...
	bsp_gpio_init(GPIOB,SYS_GPIO_PIN5,SYS_GPIO_MODE_OUT,SYS_GPIO_OTYPE_PP,SYS_GPIO_SPEED_FAST,SYS_GPIO_PUPD_PU);				//PB5 推挽输出,控制背光
	bsp_gpio_init(GPIOF,SYS_GPIO_PIN10,SYS_GPIO_MODE_AF,SYS_GPIO_OTYPE_PP,SYS_GPIO_SPEED_HIGH,SYS_GPIO_PUPD_PU);				//PF10		
	bsp_gpio_init(GPIOG,3<<6|1<<11,SYS_GPIO_MODE_AF,SYS_GPIO_OTYPE_PP,SYS_GPIO_SPEED_HIGH,SYS_GPIO_PUPD_PU);			//PG6/7/11
