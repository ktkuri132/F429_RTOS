...
uint8_t ft5206_init(void)
{
    ...
    /* RST引脚模式设置, 推挽输出, 上拉 */
    bsp_gpio_set(FT5206_RST_GPIO_PORT, FT5206_RST_GPIO_PIN,
                 SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_MID, SYS_GPIO_PUPD_PU);

    /* INT引脚模式设置, 输入模式, 上拉 */
    bsp_gpio_set(FT5206_INT_GPIO_PORT, FT5206_INT_GPIO_PIN,
                 SYS_GPIO_MODE_IN, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_MID, SYS_GPIO_PUPD_PU);

    
    ct_iic_init();      /* 初始化电容屏的I2C总线 */
    FT5206_RST(0);      /* 复位 */
    bsp_systick_delay_ms(20);
    FT5206_RST(1);      /* 释放复位 */
    bsp_systick_delay_ms(50);
    temp[0] = 0;
...
uint8_t ft5206_scan(uint8_t mode)
{
    ...
    if ((t % 10) == 0 || t < 10)   /* 空闲时,每进入10次CTP_Scan函数才检测1次,从而节省CPU使用率 */
    {
        ...
        if ((sta & 0XF) && ((sta & 0XF) < 6))
        {
            ...
            bsp_systick_delay_ms(4);                            /* 必要的延时，否则老是认为有按键按下 */
